...
- `pacifica-*.jpg` — Original Pacifica field photography in three formats: `.photo-band` full-bleed strips (wide frames), `.photo-figure` centered portrait figures (vertical frames — Tom prefers these over force-cropping verticals into bands), and the sessions `.landscape-figures` duo. **Each frame is used exactly once site-wide (Tom's rule: no photo repurposed across pages):** panorama (hub band), ridge-trail (sail home band), cypress-arch (method figure — must show the trail), rose-path (about figure), fork (begin figure, beside the contact form — two trails for two ways in), storm-coast + canopy-trail (sessions landscapes duo), vista (grow band), horizon (advise band), ridgecrest (one-pager hero), fog-valley (contact card). Forest frames are from **San Pedro Valley County Park** (not "Skyline ridge") — caption locations accurately; the sessions logistics copy lists Mori Point, Pedro Point Headlands, and San Pedro Valley Park as the actual venues. Source: `~\pacifica-studio\processed\` (graded originals); re-export via `~\pacifica-studio\scripts\export_web.py`. The two remaining stock photos (below) are intentional — do not swap them for iPhone shots, and do not reuse a pacifica frame on a second page.
