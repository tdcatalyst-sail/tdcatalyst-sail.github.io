...
- **The delivery templates (diagnostic report, proposal, brand guideline) do NOT live here anymore** — they were moved to the private `tdcatalyst-delivery` repo (commit 93ff9b1) so client documents are never publicly served. Their `.doc-body`/`.sheet`/`.ph` CSS remains in `/style.css`. Do not recreate client-facing documents in this public repo.

### The Deploy Kit is WITHDRAWN (2026-08-03) — do not republish without Tom's word

Tom pulled `/grow/deploy-kit/` offline: it is not ready to be published. The four pages (kit index, Adoption Matrix worksheet, twelve FDE governance questions, manager talking track) were deleted from the deployed tree, every inbound link was removed (Grow footer, Field Notes section, the seams and adoption-matrix essay CTAs, the Simulation closing pointer), the four sitemap entries were dropped, and `/grow/deploy-kit/` is robots-disallowed in case anything was cached.

**The work is not lost — it is in git history.** The last commit carrying the pages is `fba8ba4`; restore with:
...
**Standing to-do list + decision log (IMPORTANT):** a second private artifact, `https://claude.ai/code/artifact/48756bdc-3ed5-4e2e-803f-0bb139a35d9c`, is the working backlog for the practice (task state, open sign-offs, and doctrine for an adjacent venture that is deliberately NOT documented in this public repo). Any session doing marketing, copy, or build work must re-read it first and update it before ending. This CLAUDE.md holds only public-safe doctrine; the backlog artifact holds the rest. Decisions mirrored here: **email platform is Kit** (ex-ConvertKit) — once Tom creates the account, wire the Field Notes subscribe placeholder to it; until then the list does not exist (formsubmit cannot hold one). The Deploy Kit gating that was also waiting on Kit is moot for now: the kit itself is offline (see "The Deploy Kit is WITHDRAWN" above).
...
**Proposed and PENDING TOM'S SIGN-OFF (not yet on any page — do not ship without it):** the six site moves in leverage order (1 quantified case study; 2 named deliverables — *The Portfolio Verdict*, *The Adoption Reset Plan*, *The Operating Model Blueprint*, and *The Trail Diagnostic* for Sail's written output; 3 promote the "seam" sentence from the hub through-line into a standing thesis line; 4 a "What this is not" section on Grow; 5 a free-tools kit page with email capture beneath it — capture requires a real email platform first, formsubmit cannot hold a list; 6 restate the published fixed fee as a claim). **Tom's review verdicts, 2026-07-26 (signed off):** the kit is **The Deploy Kit** at `/grow/deploy-kit/` — a Grow page (brand-grow, grow nav/footer), NOT umbrella, and "field" vocabulary stays exclusive to Sail. **(Superseded 2026-08-03: the kit shipped, then Tom took it offline as not ready. It is out of the deployed tree — see "The Deploy Kit is WITHDRAWN" above. The naming and placement verdicts still hold whenever it returns.)** Named deliverable lines on the Grow engagement cards: kept. The "What this is not" foil section on Grow: rejected, do not re-add without a new sign-off. Diagnostic naming: **"Trail Diagnostic" is the surviving name** — sail index h2 says "The Trail Diagnostic: a written read in four areas", the adoption-matrix field note CTA already matches, and the preliminary-diagnostic quiz keeps its functional description (a lead instrument, not the deliverable). `templates/contour-card-studio.html` (internal Instagram card generator on the site terrain renderer; grounds navy/offwhite/seamist × accents gold/blue/red; seed log enforcing no-ground-repeats, seeds ≥500; exports 2160px PNG): approved. Moves 1, 3, 6 not yet implemented. Channel doctrine: LinkedIn at 3 posts/week in three fixed types (field dispatch / artifact / receipt), 90% value 10% marketing, never both in one post; Instagram as a field record, not a marketing feed — three rotating tile types (field frame / contour card on navy with gold contours / artifact crop), coordinate line as the cross-channel signature. Drafted post and section copy for all of this is in the artifact above. **The Instagram account exists (created 2026-07): `@pacifica.strategy.walks`** (https://www.instagram.com/pacifica.strategy.walks/) — deliberately walk-centric naming, so it is Sail's channel specifically. Tom intends to feature it on the site/Sail eventually (likely a footer or contact-card link, possibly a follow line near the field photography); not linked from any page yet — add only when Tom asks, and keep the no-photo-repeat rule between grid and site.
...
- `pacifica-*.jpg` — Original Pacifica field photography in three formats: `.photo-band` full-bleed strips (wide frames), `.photo-figure` centered portrait figures (vertical frames — Tom prefers these over force-cropping verticals into bands), and the method `.landscape-figures` duo. **Each frame is used exactly once site-wide (Tom's rule: no photo repurposed across pages):** ridge-trail (hub band — Tom picked it as the most scenic frame for the main page), cypress-arch (method figure — must show the trail), rose-path (about figure), storm-coast + canopy-trail (method landscapes duo — the method page "The landscapes" section carries all four field frames), vista (grow band), horizon (advise band), ridgecrest (one-pager hero), fog-valley (contact card), methuselah-grove (method portrait figure below the landscapes duo — the Methuselah redwood itself, shot at its base on Skyline Boulevard; filename not pacifica-prefixed because the venue isn't Pacifica; from IMG_4575, Tom OK'd captioning the venue "The Methuselah grove"). The sessions page is intentionally photo-free (its landscape photography was consolidated onto the method page). The sail home and begin pages are intentionally photo-free apart from the sail Mori Point thesis band — a weak photo is worse than none. Forest frames are from **San Pedro Valley County Park** (not "Skyline ridge") — caption locations accurately; the sessions logistics copy lists Mori Point, Pedro Point Headlands, and San Pedro Valley Park as the actual venues. Source: `~\pacifica-studio\processed\` (graded originals); re-export via `~\pacifica-studio\scripts\export_web.py`. The one remaining stock photo (below) is intentional — do not swap it for an iPhone shot, and do not reuse a pacifica frame on a second page.
